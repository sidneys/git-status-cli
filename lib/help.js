...
let print = () => {
    ...
    console.log(formatTableRow(`${chalk['bold'](Object.keys(packageJson.bin)[0])} [ ${chalk['bold']('--format')} ${chalk['underline']('output-format')} ] [ ${chalk['bold']('--cwd')} ${chalk['underline']('path/to/repository')} ] [ ${chalk['bold']('--git-path')} ${chalk['underline']('/usr/bin/git')} ]`))

    console.log(formatTextHeader('options'))

    console.log(formatTableRow(`${chalk['bold']('--format')} ${chalk['underline']('output-format')}`, `"json", "text"`))
    console.log(formatTableRow(`${chalk['bold']('--cwd')}`, `Path to Git repository`))
    console.log(formatTableRow(`${chalk['bold']('--git-path')}`, `Path to Git executable`))
